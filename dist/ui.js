...
function spark(series) {
    if (!series?.length)
        return `<div class="muted">No U<sub>t</sub> series.</div>`;
    return `
    <div class="spark">
      ${series
        .map((p) => {
...
function meterCard(m) {
    ...
    const head = m.mode === "quant_qual"
        ? `<div class="big"><span class="score">${fmt(m.E_window)}</span><span class="pill">${esc(m.band ?? "—")}</span></div>`
...
function renderReport(r) {
  ...
  `;
    const scoreBlock = mode === "quant_qual"
        ? `
      <div class="card">
        <h2>Final score</h2>
        <div class="big">
    ...
    const dimsBlock = mode === "quant_qual" && dims
        ? `
      <div class="card">
    ...
    const segments = (r.segments ?? [])
        .map((s) => {
        const share = typeof s.shareUserTurns === "number"
  ...
  `;
    const segCards = (r.segmentReports ?? [])
        ...
        const share = typeof sr.shareUserTurns === "number"
 ...
 */
export function renderReportHTML(sessionId) {
