...
export function scoreSegments(params) {
    ...
    const scoreById = new Map(params.modelTurnScores.map(s => [s.turnId, s]));
    return params.segments.map(seg => {
        const segScores = seg.turnIds
            .map(id => scoreById.get(id))
            .filter(Boolean);
        const utSeries = computeUtSeries(segScores.map(s => ({
            turnId: s.turnId,
            dims: s.dims
        })));
        ...
        // mixed=0.5
        // operational=0
        const conceptualShare = segScores.length === 0
            ? 0
        ...
        const share = seg.turnIds.length / Math.max(userTurns.length, 1);
        const qualitativeSummary = params.segmentQualSummaries?.[seg.segmentId];
        return {
