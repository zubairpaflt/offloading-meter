...
import { computeUtSeries, meanDims, computeSessionE, clamp01 } from "./compute.js";
import { scoreWithModel } from "./scoring.js";
import type { Band, Turn } from "./types.js";

const app = express();

// avoid "Number is not callable" by not using Number(...)
...
function buildTurns(text: string): Turn[] {
  ...
  const userPrefix = /^(\s*(\d+[\)\.\-]\s*)?)(u(?:ser)?\s*\d*)\s*[:\-]\s*/i;
  const asstPrefix = /^(\s*(\d+[\)\.\-]\s*)?)(a(?:ssistant)?|chatgpt)\s*\d*\s*[:\-]\s*/i;
...
app.post("/analyze", async (req: Request, res: Response) => {
  try {
    // cast body to any so Vercel TS doesn’t complain
